	...
	*/
	...
	function i(t, n = globalThis, s = ["*"]) {
		n.addEventListener("message", function o(u) {
			...
			const { id: l, type: p, path: g } = Object.assign({ path: [] }, u.data), f = (u.data.argumentList || []).map(h);
			let m;
			try {
				...
				switch (p) {
					case "GET":
						...
						break;
					case "SET":
						n[g.slice(-1)[0]] = h(u.data.value), m = !0;
						break;
					case "APPLY":
						m = r.apply(n, f);
						break;
	...
	function E(e) {
		const t = Object.assign({}, e), n = e.pair ? Object.assign({}, e.pair) : {}, r = n?.target_token || "", a = t.chain || "", s = n?.token0_address || "";
		return {
			...t,
			...n,
			id: `${r}-${a}`,
			...
			logo_url: t.logo_url || (r === s ? n?.token0_logo_url : n?.token1_logo_url)
		};
	}
